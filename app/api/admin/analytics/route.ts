...
export async function GET(request: NextRequest) {
  try {
    ...
    const [totalUsers, newUsersPeriod, activeUsersPeriod, totalScripts, totalVideos, scriptsPeriod, videosPeriod] = await Promise.all([
      ...
      prisma.scripts.count({ where: { created_at: { gte: start } } }),
      prisma.scripts.count({ where: { created_at: { gte: start }, generated_video_url: { not: null } } }),
    ])
    ...
    const prevScripts = await prisma.scripts.count({
      where: { created_at: { gte: prevStart, lte: prevEnd } },
    })
    const prevVideos = await prisma.scripts.count({
      where: { created_at: { gte: prevStart, lte: prevEnd }, generated_video_url: { not: null } },
    })
    ...
    const [scriptsList, videosList] = await Promise.all([
      prisma.scripts.findMany({ where: { created_at: { gte: sevenDaysAgo } }, select: { created_at: true } }),
      prisma.scripts.findMany({ where: { created_at: { gte: sevenDaysAgo }, generated_video_url: { not: null } }, select: { created_at: true } }),
    ])
    ...
    scriptsList.forEach((s) => {
      const dateStr = (s.created_at ?? new Date()).toISOString().split("T")[0]
      if (contentByDay[dateStr]) contentByDay[dateStr].scripts += 1
    })
    videosList.forEach((v) => {
      const dateStr = (v.created_at ?? new Date()).toISOString().split("T")[0]
